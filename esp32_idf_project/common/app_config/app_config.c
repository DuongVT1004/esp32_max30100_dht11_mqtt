...
static const int WIFI_SENT_DONE_BIT = BIT2;
...
void ap_start()
{
    wifi_config_t wifi_config = {
        .ap = {
            .ssid = "Balotelli",
            .ssid_len = strlen((char*)"Balotelli"),
            .channel = 1,
            .password = "23092000",
            .max_connection = 4,
...
void wifi_data_callback(char *data, int len)
{
    ...
    printf("->ssid:  %s\n", ssid);
    printf("->pass:  %s\n", pass);
    xEventGroupSetBits(s_wifi_event_group, WIFI_SENT_DONE_BIT);
}
...
void app_config(void)
{
    ...
    if(!provisioned)
    {
        ...
        else if(provision_type == PROVISION_ACCESSPOINT)
        {
            ...
            xEventGroupWaitBits(s_wifi_event_group, WIFI_SENT_DONE_BIT, false, true, portMAX_DELAY); 
            stop_webserver();
            esp_wifi_disconnect();
            esp_wifi_stop();
            wifi_config_t wifi_config = {
